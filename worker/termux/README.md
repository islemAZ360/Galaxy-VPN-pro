...
- **Tapping a shortcut shows "Termux requires Display over other apps"** — grant
  ...
  *Display pop-up windows* + *…while running in the background*, and **Autostart**.
  ...
  sources"), first lift it via the **⋮** menu → **Remove (permission) restrictions**
  (Xiaomi: on the app-info page; Realme/ColorOS: on the app's **Permissions** page),
  then enable the toggle. No-permission fallback: open Termux and just type
  `wifi` or `lte` (aliases installed by setup.sh).
